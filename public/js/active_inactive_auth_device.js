...
function changeDeviceStatus(device_status_change_path,title,text,token,type,id, status)
{
    swal({
        title: title,
        type: type,
        ...
        if (isConfirm) {
            deleteRequest(device_status_change_path,id,token, status);
        } 
    });
}
...
function checkLength(delete_id)
{
    ...
    if(0 == selected_length){
        ...
        deleteRecord(device_status_change_path,title,text,token,type,id, status)
    }
}
//this function  is used to call delete record
function deleteRequest(device_status_change_path,id,token, status)
{
    $.ajax({
        url: device_status_change_path,
        type:'post',
        dataType:'json',
